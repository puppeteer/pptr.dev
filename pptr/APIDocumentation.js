...
export class APIDocumentation {
  ...
  createOutline() {
    const sectionItems = this.sections.map(apiSection => html`
      <li class=table-of-contents-entry>${apiSection.createTableOfContentsElement()}</li>
    `);

    const classItems = this.classes.map(apiClass => {
      const classEntries = [...apiClass.events, ...apiClass.namespaces, ...apiClass.methods].map(entry => html`
...
export class APIEntry {
  ...
  createTableOfContentsElement() {
    ...
    if (this.sinceVersion) {
      const className = this.api.version === this.sinceVersion ? 'pptr-new-api' : '';
      fragment.appendChild(html`
        <pptr-api-since class=${className}>${this.sinceVersion}</pptr-api-since>
      `);
    }
    if (this.untilVersion) {
      fragment.appendChild(html`
        <pptr-api-until>${this.untilVersion}</pptr-api-until>
      `);
    }
    return fragment;
  }
...
export class APIClass extends APIEntry {
  ...
  _appendTableOfContents() {
    const addSection = (title, entities) => {
      const liElements = entities.map(entity => html`
        <li class=table-of-contents-entry>${entity.createTableOfContentsElement()}</li>
      `);
      this.element.appendChild(html`
        <h4>${title}</h4>
        <ul class=pptr-table-of-contents>${liElements}</ul>
      `);
    };
